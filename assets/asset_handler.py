...
class NewAsset(object):
    ...
    def add_to_new_assets_zone(self):
        ...
        defaults = {
            ...
            'cpu_physical_count': self.data.get('cpu_physical_count'),
...
def log(log_type, msg=None, asset=None, new_asset=None, user=None):
    """
    记录日志
    """
...
class UpdateAsset:
    ...
    def __init__(self, user, asset, report_data):
        self.user = user
        self.asset = asset
    ...
    def _server_update(self):
        try:
            ...
            log('update_failed', msg=e, asset=self.asset, user=self.user)
            print(e)
            return False
        else:
            # 添加日志
...
class ApproveAsset:
    ...
    审批资产并上线。
    """
    def __init__(self, user, asset_id):
        self.user = user
    ...
    def asset_upline(self):
        # 为以后的其它类型资产扩展留下接口
        print(self.new_asset.asset_type,type(self.new_asset.asset_type))
    ...
    def _server_upline(self):
        ...
        try:
            ...
            asset.delete()
            log('approve_failed', msg=e, new_asset=self.new_asset, user=self.user)
            print(e)
            return False
        else:
            # 添加日志
            log("upline", asset=asset, user=self.user)
            print("新服务器上线!")
            return True
    ...
    def _create_asset(self):
        ...
        asset = models.Asset.objects.create(asset_type=self.new_asset.get_asset_type(),
